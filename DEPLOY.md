...
| PUT | `/api/semesters` | 관리자 | 학기·강의·도서 정보 수정 (기존 항목만, 생성/삭제는 아래 참조). 다른 관리자가 먼저 저장했으면 409 |
...
학기마다 `version` 값이 있고, 저장 시 화면이 불러왔던 version을 함께 보냅니다. 그 사이 다른
관리자가 저장했다면 **아무것도 저장되지 않고** 409와 함께 최신 내용이 돌아옵니다. 화면은
자동으로 최신 내용으로 갱신되므로, 수정하려던 부분을 다시 반영한 뒤 저장하면 됩니다.

일부 학기만 어긋난 경우에도 전체가 취소됩니다(부분 저장 없음). 강의를 추가·삭제해도 해당
학기의 version이 올라갑니다.
